...
import com.project.ecommercestore.entities.OrderItem;
...
public class StripePaymentGateway implements PaymentGateway{
    ...
    @Override
    public Optional<PaymentResult> parseWebhookRequest(WebhookRequest rq) {
        try {

            var payload = rq.getPayload();
            var signature = rq.getHeaders();
            var event = Webhook.constructEvent(payload, signature, webhookSecretKey);

            return switch (event.getType()) {
                case "payment_intent.succeeded" -> Optional.of(new PaymentResult(extractOrderId(event), PaymentStaus.PAID));

                case "payment_intent.payment_failed" -> Optional.of(new PaymentResult(extractOrderId(event), PaymentStaus.FAILED));
    ...
    private Long extractOrderId(Event event) {
        var stripeObj = event.getDataObjectDeserializer().getObject().orElseThrow(
                () -> new PaymentException("Invalid Stripe Object. Check SDK and API version")
        );
